...
public class InteractManager : MonoSingleton<InteractManager>
{
    ...
    public InteractImage AimObject => aimObject;

    private Camera _mainCamera;
    private BoxCollider2D _playerCollider;

    private BaseTower _lastAttackAreaOwner;
    ...
    void Update()
    {
        ...
        if (hit != null)
        {
            ...
            {
                aimObject.transform.position = (hit.attachedRigidbody != null)
                    ? hit.attachedRigidbody.transform.position
    ...
    {
        float range = (interactable is OreController) ? 0.5f : 5.0f;
    ...
    public void NotifyInteract(BaseTower tower)
    {
        if (_lastAttackAreaOwner != null && _lastAttackAreaOwner != tower)
            _lastAttackAreaOwner.OffAttackArea();

        _lastAttackAreaOwner = tower;
    }
