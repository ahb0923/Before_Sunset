...
4 or 5 IDs, separated by underscores. For example: "fbas_14head_headscarf_00b_e"
	...
	ID 5 (special ID: hair replacement/removal)
		The 5th string is a special code only used on some specific parts. On all other parts, there will only be 4 IDs.
		Right now, there are two uses of the special ID "_e": in hats and hairstyles that's can't coexist.
		
		For hats, it means the hat cannot coexist with any hairstyles (like the headscarf).
		When a character equips a hat with a "_e" at the end, then the hair sprite in layer "13hair" (ID 2) should be removed/hidden.
		
		For hairstyles, it means the hair itself cannot coexist with any hat (like the mohawk).
		When the character equips a hairstyle with a "_e" at the end, then it should hide itself when the character wears any hat.
